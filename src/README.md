...
`GEM300_Log_Analyzer_vX.Y.Z.zip`을 내려받아 압축을 해제합니다. EXE 파일은
저장소 소스에 포함하지 않습니다.
...
- 대용량 로그 이동 최적화: 전체 인덱스 직접 조회와 최대 10,000행 윈도우 렌더링
- 디스크 기반 원문 저장: 전체 원문을 메모리에 유지하지 않고 캐시 사이드카에 저장한 뒤 화면 표시와 검색 시 필요한 범위만 읽기
- 단일 대용량 파일 병렬 파싱: 로그 메시지 경계를 보존한 byte 구간을 최대 4개 프로세스로 동시 처리
- 영구 검색 인덱스: 파일과 분석 옵션이 같으면 SQLite 키워드 인덱스를 다음 실행에서도 재사용
- 지연 DB 부가정보: 로그에 실제 등장한 CEID/RPTID만 조회하고, CEID/VID 이름은 현재 화면과 선택 상세에서만 결합해 대용량 로그의 전수 후처리와 재인덱싱 방지
- 대용량 파일 보호: 단일 대용량 파일은 최대 4개 프로세스로 분할하고 여러 대용량 파일은 동시 작업 수를 제한
