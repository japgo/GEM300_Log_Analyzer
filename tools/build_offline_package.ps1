...
if ($PythonCommand) {
    & $PythonCommand -m pip download -r (Join-Path $RepoRoot "requirements.txt") -d $WheelsRoot
}
elseif (Get-Command py -ErrorAction SilentlyContinue) {
    & py -m pip download -r (Join-Path $RepoRoot "requirements.txt") -d $WheelsRoot
}
else {
    & python -m pip download -r (Join-Path $RepoRoot "requirements.txt") -d $WheelsRoot
}

Copy-Item -LiteralPath (Join-Path $RepoRoot "tools\install_offline.ps1") -Destination (Join-Path $TargetRoot "install_offline.ps1") -Force
Copy-Item -LiteralPath (Join-Path $RepoRoot "tools\README_OFFLINE_INSTALL.md") -Destination (Join-Path $TargetRoot "README_INSTALL.md") -Force
